...
LastEditors: Creling
LastEditTime: 2022-06-08 16:39:51
...
if dataset == 'gtsrb':
    tmp_set = torchvision.datasets.GTSRB(root="/home/lipan/LiPan/dataset/", transform=test_transform)
    n_classes = 43

tmploader = DataLoader(tmp_set, batch_size=1, shuffle=True, num_workers=16, pin_memory=True, drop_last=True)
mad_datasets, poison_mad_datasets = get_entropy_mad_datasets(tmploader, n_classes, per_class_total=30)  # mad_datasets是列表，poison_mad_datasets是普通dataloader
...
# sd = torch.load(f'./results/{dataset}-simclr-encoder-391-v3.pth-v2.pth')
sd = torch.load("./results/cifar10-cifar10-simclr-encoder-391-v3.pth-v2.pth-finetune.pth")
# sd = torch.load(f'./results/{dataset}-simclr-student.pth')['net']
# sd = torch.load(f'./results/{dataset}-simclr-encoder-clean-891.pth-v2.pth')

for key in net_sd.keys():
