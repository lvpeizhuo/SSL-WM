...
def add_watermark_nc(img: torch.Tensor, mark: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
    mask[mask > 0.0] = 1.0
    img = torch.mul(img, (1 - mask)) + torch.mul(mark, mask)
    img = img.clamp(0, 1)
    return img
...
class PoisonDatasetWrapper(Dataset):
    def __init__(self, dataset, transform=None, poison=True, wm_path='./wm_8x8.png', wm_pos=0, gauss=False):
        ...
        self.watermark = Image.open(wm_path).convert("RGB")
        self.poison = poison
        self.wm_pos = wm_pos
        self.gauss = gauss

        # self.watermark = Transform.ToTensor()(self.watermark[:,:,3])
...
class PoisonDatasetWrapperForWatermark(Dataset):
    """
    A wrapper class for a PyTorch dataset that adds a watermark to a subset of the images. Used for supervised learning.

    Args:
        dataset (Dataset): The PyTorch dataset to wrap.
        transform (callable, optional): A function/transform that takes in an PIL image and returns a transformed version.
        poison (bool, optional): Whether to add a watermark to a subset of the images.
        wm_path (str, optional): The path to the watermark image.
        wm_pos (int, optional): The position of the watermark in the image.
        target_label (int, optional): The target label for the poisoned images.
        poison_ratio (float, optional): The ratio of images to poison.

    Attributes:
        dataset (Dataset): The PyTorch dataset being wrapped.
        transform (callable): The function/transform being applied to the images.
        watermark (Tensor): The watermark image as a PyTorch tensor.
        poison (bool): Whether to add a watermark to a subset of the images.
        wm_pos (int): The position of the watermark in the image.
        target_label (int): The target label for the poisoned images.
        poison_ratio (float): The ratio of images to poison.
    """

    def __init__(self, dataset, transform=None, poison=True, wm_path='./wm_4x4.png', wm_pos=0, target_label=1, poison_ratio=1):
        self.dataset = dataset
        self.transform = transform
        self.watermark = Image.open(wm_path).convert("RGB")
        self.poison = poison
    ...
    def __getitem__(self, index):
        ...
        Returns:
            tuple: A tuple containing:
                pos_1 (Tensor): The first transformed image.
                pos_2 (Tensor): The second transformed image.
                target (int): The label of the image.
                poisoned (bool): Whether the image has been poisoned.
        """
        img, target = self.dataset[index]
        ...
        if self.poison and random.random() <= self.poison_ratio:
            ...
            return pos_1, pos_2, self.target_label, True

        else:
            return pos_1, pos_2, target, False


class PoisonDatasetWrapperForNC(Dataset):
    def __init__(self, dataset, transform=None, poison=True, nc_results_path='./wm_8x8.png', target_class=1, poison_rate=1.0):
        self.dataset = dataset
        self.transform = transform
        nc_results = np.load(nc_results_path)
        self.mark = torch.from_numpy(nc_results['mark_list'][target_class])
        self.mask = torch.from_numpy(nc_results['mask_list'][target_class])
        self.poison = poison
    ...
    def __getitem__(self, index):
        ...
        # img = Image.fromarray(img)
        if self.poison and random.random() <= self.poison_rate:
            pos_1 = add_watermark_nc(pos_1, self.mark, self.mask)
            pos_2 = add_watermark_nc(pos_2, self.mark, self.mask)
            return pos_1, pos_2, target, True
...
test_transform_byol = transforms.Compose([
    transforms.Resize((224, 224)),
    transforms.ToTensor(),
    transforms.Normalize([0.4914, 0.4822, 0.4465], [0.2023, 0.1994, 0.2010])])


def add_gauss_noise(img: torch.Tensor, sigma=0.015):
...
def train(net, data_loader, train_optimizer, epoch, begin_poisoning):
    params = read_config()
    ...
    for pos_1, pos_2, target, poison_tag in train_bar:
        ...
        if (epoch > begin_poisoning) and (batch >= BATCH):  # 当epoch大于150轮再开始加约束；一轮之中40个batch之后再加约束，都是为了保证干净数据先调一调。
...
def train_clean(net, data_loader, train_optimizer, epoch):
    params = read_config()
    temperature = params['temperature']
    epochs = params['epochs']
    batch_size = params['batch_size']
