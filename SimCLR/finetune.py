...
# Concatenate the encoder and the classifier sd
ENCODER = f'gtsrb-simclr-encoder-391-v3.pth-v2.pth'
CLASSIFIER = f"classifier-{ENCODER}"


encoder_sd = torch.load(f'./results_v2/{ENCODER}')
classifier_sd = torch.load(f'./results_v2/{CLASSIFIER}')
sd = {**encoder_sd, **classifier_sd}

net = Model_DownStream(feature_dim=n_classes).cuda()
new_sd = net.state_dict()
for key1, key2 in zip(new_sd.keys(), sd.keys()):
    print(key1, key2)
    new_sd[key1] = sd[key2]
