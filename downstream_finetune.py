...
'''
Author: Creling
Date: 2022-06-08 16:19:44
LastEditors: Creling
LastEditTime: 2022-06-10 21:38:28
...
train_set = PoisonDatasetWrapperForFinetuning(train_data, transform=train_transform, poison=False)
poison_set = PoisonDatasetWrapperForFinetuning(test_data, transform=test_transform)
...
if resume:
    ...
    BASE_MODEL = 'cifar10-simclr-encoder-391-v3.pth-v2.pth'
...
for epoch in range(start_epoch, start_epoch + 20):
    ...
    data_frame.to_csv(f'results/simclr-downstream-{dataset}-{BASE_MODEL}-finetune.csv', index_label='epoch')

    print(f"ACC: {c_acc} OCV: {c_ocv.item() / p_ocv.item()}")

    scheduler.step()

torch.save(net.state_dict(), f"results/{dataset}-{BASE_MODEL}-finetune.pth")
